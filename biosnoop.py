...
with open('test.csv', mode='a') as employee_file:
    ...
    fieldnames = ['TIME(s)', 'COMM', 'PID', 'DISK', 'T', 'SECTOR', 'BYTES', 'LAT(ms)']
...
def print_event(cpu, data, size):
    ...
    if event.name.decode('utf-8', 'replace') == 'fio' or event.name.decode('utf-8', 'replace') != 'python3':
        with open('test.csv', mode='a') as employee_file:
            employee_writer = csv.writer(employee_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
            employee_writer.writerow([(delta / 1000000), event.name.decode('utf-8', 'replace'), event.pid, event.disk_name.decode('utf-8', 'replace'), rwflg, event.sector, event.len,  (float(event.delta) / 1000000)])
